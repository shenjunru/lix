...
$break = {},
_pseudoModify = function(pseudo){
  if(pseudo.value){
    switch(pseudo.name){
      case 'not':{
				pseudo.value = _convertDetector(pseudo.value);
				break;
			}
      case 'nth-child':{
        // parse equations like 'even', 'odd', '5', '2n', '3n+2', '4n-1', '-n+6'
        var match = PATTERN.NTH.exec(
          pseudo.value == 'even' && '2n' ||
          pseudo.value == 'odd' && '2n+1' ||
          !/\D/.test(pseudo.value) && '0n+' + pseudo.value ||
          pseudo.value);
...
_convertDetector = function(str){
	var ret = {}, i = 0, l = str.length, _str, m1, m2;
  while (i < l) {
    ...
    switch (m1[1]) {
      ...
      case '[':{
				...
				if (m2) {
					ret['[]'] = ret['[]'] || [];
					ret['[]'].push(_attrModify({
						name: m2[1],
						match: m2[2] || '',
						value: m2[5] || m2[4] || m2[3],
						target: null
					}));
				}
				break;
...
_convertSelector = function(str){
  var ret = [], i = 0, l = str.length, g = 0, ng = true, _str, m1, m2, sl, _sl;
  while (i < l) {
    _str = str.slice(i, l), m1 = PATTERN.CHAR.exec(_str);
    if (!m1) break;
    sl = new CSSSelector(), sl.group = g;
    switch (m1[1]) {
      // Filter
      case '>':
      case '~':
      ...
      case '.':
        ...
        if (m2) {
          ...
          sl.detector['.'] = sl.detector['.'] || [];
          sl.detector['.'].push(m2[2]);
        }
        break;
      // PSEUDO
      case ':':
        ...
        if (m2) {
          sl = _sl || sl;
          sl.detector[':'] = sl.detector[':'] || [];
          sl.detector[':'].push(_pseudoModify({
            name: m2[1],
            value: m2[2]
          }));
        }
...
var CSSQuery = function(selector, context){
  ...
  if (typeof(selector) === 'string') {
    ...
    var stack = _convertSelector(selector), context = new this.constructor(this.context), i = 0, frag;
    while ((frag = stack[i++]))
...
FILTER: {
  '#': function(result, selector, context){
    ...
    if (node && _tag(selector, node) && _detector(selector, node, null)) {
      if (selector.prev == null) result.push(node);
      else {
        if(_contains(context, node)) result.push(node);
      }
      throw $break;
    }
  },
  ...
  '>': function(result, selector, context){
    ...
    while ((node = children[i++])) if(node.nodeType == 1 && _tag(selector, node) && _detector(selector, node, i)) result.push(node);
  },
  '+': function(result, selector, context){
    var i = 0, node = context;
    while ((node = node.nextSibling)) 
      if (node.nodeType == 1) {
        if (_tag(selector, node) && _detector(selector, node, i++)) result.push(node);
        break;
      }
  },
  '~': function(result, selector, context){
    var i = 0, node = context;
    while ((node = node.nextSibling))
      if(node.nodeType == 1 && _tag(selector, node) && _detector(selector, node, i++)) result.push(node);
  }
},
// Detectors
DETECTOR: {
...
//    if (node.parentNode) return node.parentNode.childNodes.length === 1;
    var tmp = node;
  ...
  not: function(node, value, index){return !_detector({detector:value}, node, index);}
}
};
